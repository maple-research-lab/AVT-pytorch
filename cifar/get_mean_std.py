...
mean = numpy.mean(all_coeffs, axis=0, keepdims=True)
std = numpy.std(all_coeffs, axis=0, keepdims=True)
print(mean)
print(std)

mean = numpy.asarray([[0,0,16,0,0,16,5.5e-4,5.5e-4]], dtype='float32')
std = numpy.asarray([[0.763,0.763,17,0.763,0.763,17,6.12e-3,6.12e-3]], dtype='float32')

all_coeffs_norm = (all_coeffs - mean) / std
print(numpy.max(all_coeffs_norm, axis=0))
print(numpy.min(all_coeffs_norm, axis=0))
