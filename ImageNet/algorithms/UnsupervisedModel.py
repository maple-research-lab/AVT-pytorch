...
class UnsupervisedModel(Algorithm):
    ...
    def allocate_tensors(self):
        ...
        self.tensors['dataY'] = torch.FloatTensor()
        self.tensors['labels'] = torch.FloatTensor()
    ...
    def process_batch(self, batch, do_train=True):
        ...
        self.tensors['dataY'].resize_(batch[1].size()).copy_(batch[1])
        self.tensors['labels'].resize_(batch[2].size()).copy_(batch[2])
        dataX = self.tensors['dataX']
        dataY = self.tensors['dataY']
        ...
        dataY_var = torch.autograd.Variable(dataY, volatile=(not do_train))
        labels_var = torch.autograd.Variable(labels, requires_grad=False)
        #********************************************************

        #************ FORWARD THROUGH NET ***********************
        f1, f2, output_mu, output_logvar = self.networks['model'](dataX_var, dataY_var)
        output_logvar = output_logvar / 1000.
        std_sqr = torch.exp(output_logvar)
        batch_size = dataX_var.size(0)
        #********************************************************

        #*************** COMPUTE LOSSES *************************
        record = {}
        loss_total = (torch.sum(output_logvar) + torch.sum((output_mu - labels_var)*(output_mu - labels_var) / (std_sqr + 1e-4))) / batch_size
        record['loss'] = loss_total.item()
        loss_matrix = self.criterions['loss'](output_mu, labels_var)
        record['loss_matrix'] = loss_matrix.item()
        #********************************************************
